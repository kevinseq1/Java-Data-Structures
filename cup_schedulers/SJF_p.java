...
public class SJF_p {
	...
	//Constructors
	public SJF_p(Process[] processArray){
	...
	public void setTheWaitTime(){
		...
		public void calculateAverageWaitTime(){
			...
			for(int i=0; i<processArray.length; i++) {
				if(i == 0) {
					totalWaitTime += (processArray[i].getWaitTime() - 1);
				} else {
					totalWaitTime += (processArray[i].getWaitTime() - processArray[i].getArrivalTime());
				}
				
			}
			averageWaitTime = totalWaitTime / processArray.length;
			System.out.println("\nThe Average Waiting Time for SJF_p is: " + averageWaitTime);
		}
